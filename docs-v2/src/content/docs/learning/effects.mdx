...
The effect runs immediately and prints `The count is now 0`.
...
To stop the effect, call `dispose()`:
...
If you want to create an effect without running it immediately, use `Effect.manual` and call `run()` later.
