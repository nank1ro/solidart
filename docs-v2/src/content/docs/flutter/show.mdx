...
The `Show` widget takes a function that returns a `bool`.
Signals are callable, so you can pass them directly (for example, `when: loggedIn`)
or use `.value` if you prefer.
