...
All [Computed](/api-docs/computed) signals are ReadSignals.
...
count.value++; // prints "The count is 1"
```
...
count.observe((previousValue, value) {
  print("The count is $value");
}, fireImmediately: true); // prints "The count is 0"
```
