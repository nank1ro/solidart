...
You may to subscribe only to a sub-field of a `Signal` value.
```dart
// sample User class
class User {
  const User({
...
You can also transform the type like:
```dart
final counter = createSignal(0); // Signal<int>
final isGreaterThan5 = counter.select((value) => value > 5); // ReadableSignal<bool>
```
