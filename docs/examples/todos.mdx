...
Each tab contain the number of todos present.
```dart
...
In order to react to the active filter is uses the `observe` extension on `BuildContext` that subscribes to the signal provided and rebuilds every time the value changes.
