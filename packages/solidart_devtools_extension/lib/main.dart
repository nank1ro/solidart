...
class FooDevToolsExtension extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const DevToolsExtension(child: Signals());
  }
}
            ...
            return ShadResizablePanelGroup(
              ...
              children: [
                ...
                ShadResizablePanel(
                  ...
                  child: SignalBuilder(
                    builder: (context, _) {
                      if (selectedSignalId.value == null) {
                        return const SizedBox();
                      }
                      final signal = filteredSignals.value
                          .firstWhereOrNull(
                            (element) => element.key == selectedSignalId.value,
                          )
                          ?.value;
                      if (signal == null) return const SizedBox();
                      return KeyedSubtree(
                        key: ValueKey(selectedSignalId.value),
                        child: LayoutBuilder(
...
class ParameterView extends StatefulWidget {
  const ParameterView({super.key, required this.name, this.value});
...
class _ParameterViewState extends State<ParameterView> {
  ...
  Widget build(BuildContext context) {
    ...
    return ShadTheme(
      data: ShadTheme.of(
        context,
      ).copyWith(textTheme: ShadTextTheme(family: kDefaultFontFamilyMono)),
      child: Builder(
        builder: (context) {
          return Column(
            children: [
      ...
      return Padding(
        ...
        child: Column(
          children: map.entries
              .map((e) => ParameterView(name: e.value.key, value: e.value))
              .toList(),
        ),
      );
    }
